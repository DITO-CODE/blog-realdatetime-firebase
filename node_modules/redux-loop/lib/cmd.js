...
exports.executeCmd = exports.isCmd = undefined;
...
var executeCmd = exports.executeCmd = function executeCmd(cmd, dispatch, getState) {
  switch (cmd.type) {
    case cmdTypes.RUN:
...
var list = function list(cmds) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (!options.testInvariants) {
      ...
      (0, _utils.throwInvariant)((typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object', 'Cmd.list: second argument to Cmd.list must be an options object');
    }
  } else if (options.testInvariants) {
